...
import { read, write, remove } from "../storage/local-storage.js";
...
export const login = async (email, password) => {
  ...
  if (
    !user ||
    (await digest(`${user.passwordSalt}:${password}`)) !== user.passwordHash
  )
    throw new Error("Correo o contraseña incorrectos.");
  write(KEYS.session, { userId: user.id, startedAt: now() });
  return user;
};
export const register = async ({ name, email, password, community }) => {
  ...
  const user = {
    ...
    email,
    passwordSalt,
    passwordHash: await digest(`${passwordSalt}:${password}`),
    role: "user",
    avatar: "img/default-avatar.svg",
    bio: "",
    community: community.trim() || "Villa Abecia",
    createdAt: now(),
    updatedAt: now(),
  };
...
export const updateCurrentUser = (changes) => {
  ...
  const list = users().map((u) =>
    u.id === me.id ? { ...u, ...changes, updatedAt: now() } : u,
  );
  write(KEYS.users, list);
  return list.find((u) => u.id === me.id);
};
export const updateUser = (userId, changes) => {
  const list = users().map((u) =>
    u.id === userId ? { ...u, ...changes, updatedAt: now() } : u,
  );
  write(KEYS.users, list);
